...
if ! grep -q '{{ .Values.Port }}' ${DIRECTORY}/templates/Ingress.yaml ||
	...
	! grep -q '{{ .Values.Domain }}' ${DIRECTORY}/templates/Ingress.yaml ; then
	echo "You must use the name Name, Domain and Ports from the values.yaml in order for the chart to be able to be installed!"
	exit 1
fi
